...
//  - listReportTemplates: list available reports
//  - generateReport(report_code, format, filters): runs query, exports, uploads, audits
...
export const listReportTemplates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      ...
      .select("code, name, description, admin_only, sort_order")
      .order("sort_order");
    if (error) throw new Error(error.message);
    // Filter admin_only based on role
...
export const generateReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const rows = await queryFn(context.supabase, data.filters);

    // Get caller display name
    const { data: profile } = await context.supabase.from("profiles")
      .select("full_name").eq("id", context.userId).single();
    const by = profile?.full_name ?? "Usuario";

    let bytes: Uint8Array;
    if (data.format === "csv") bytes = toCSV(spec, rows);
    else if (data.format === "xlsx") bytes = await toXLSX(spec, rows, data.filters, by);
    else bytes = await toPDF(spec, rows, data.filters, by);
    ...
    return { url: signedUrl, path, rows_count: rows.length, format: data.format };
  });
...
export const savePreset = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    report_code: z.string(), name: z.string().min(1), filters: z.record(z.any()),
  }).parse(d))
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase.from("saved_report_filters").insert({
      user_id: context.userId, report_code: data.report_code,
      name: data.name, filters_json: data.filters,
    }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const deletePreset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("saved_report_filters")
