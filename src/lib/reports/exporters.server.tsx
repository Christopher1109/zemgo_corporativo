...
export async function toXLSX(spec: ReportSpec, rows: any[], filters: ReportFilters, by: string): Promise<Uint8Array> {
  const ExcelJS = (await import("exceljs")).default;
  const wb = new ExcelJS.Workbook();
  wb.creator = "HOPE Consulting";
  wb.created = new Date();
  ...
  // Header row
  const headerRow = ws.addRow(spec.columns.map((c) => c.label));
  ...
  rows.forEach((r) => {
    const row = ws.addRow(spec.columns.map((c) => {
      const v = r[c.key];
      if (c.format === "money") return Number(v ?? 0);
      if (c.format === "date" && v) return new Date(v as any);
      return v ?? "";
    }));
    spec.columns.forEach((c, i) => {
  ...
  if (spec.totals?.length) {
    ...
    const row = ws.addRow(spec.columns.map((c, i) =>
      i === 0 ? "TOTAL" : c.key in totals ? totals[c.key] : ""
    ));
    row.font = { bold: true };
    spec.columns.forEach((c, i) => {
      if (c.format === "money") row.getCell(i + 1).numFmt = '"$"#,##0';
    });
  }
...
export async function toPDF(spec: ReportSpec, rows: any[], filters: ReportFilters, by: string): Promise<Uint8Array> {
  return await renderPdfToBytes(<ReportTablePdf spec={spec} rows={rows} filters={filters} generatedBy={by} />);
}

// Upload helper — uses service-role admin client.
export async function uploadReport(
  ...
  const contentType =
    ...
    "application/pdf";
  const { error: upErr } = await supabaseAdmin.storage.from("reports")
    .upload(path, bytes, { contentType, upsert: false });
  if (upErr) throw upErr;
  const { data: signed, error: sErr } = await supabaseAdmin.storage.from("reports")
