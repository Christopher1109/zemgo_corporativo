// Demo seed for the kickoff demo. Generates realistic Mexican data tagged with
// metadata.is_demo=true so it can be wiped without touching real records.
//
// Endpoints:
//   - getSeedDemoCounts(): counts of currently-tagged demo rows by table
//   - runSeedDemo():   inserts demo data (idempotent via clear-first option)
//   - clearSeedDemo(): deletes every row where metadata.is_demo='true'
//
// All three require the caller to be a super-admin.
...
export const runSeedDemo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // 4. Clients (20 per program = 60 total)
    ...
    // 5. client_programs (enroll each client in its program)
    {
    ...
    // 6. Policies (1 per client). Use next_policy_folio RPC for unique folios.
    ...
    for (const c of allClients) {
      ...
      const startOffset = randInt(-330, -10); // already-started policies
      ...
      const { data: pol, error } = await supabaseAdmin.from("policies").insert({
        ...
        program_id: c.program_id,
        client_id: c.id,
        issue_date: isoDate(start),
        start_date: isoDate(start),
        end_date: isoDate(end),
        sum_insured: sumInsured,
        deductible: Math.round(sumInsured * 0.05),
        premium,
        status: "active",
    ...
    // 7. Beneficiaries (1-3 per policy, sum to 100)
    {
      const rows: any[] = [];
    ...
    // 9. Payments (monthly schedule from start to today)
    {
      const rows: any[] = [];
      for (const p of policies) {
        ...
        const monthsSinceStart = Math.min(
          12,
          Math.max(1, Math.floor((today.getTime() - p.start.getTime()) / (30 * 86400000))),
        );
        for (let m = 0; m < monthsSinceStart; m++) {
          ...
          if (m < monthsSinceStart - 2) {
            // older months: 90% paid
            if (rand100 < 0.9) {
              status = "paid";
              paidAt = shiftDays(dueDate, randInt(-3, 5)).toISOString();
              paidAmount = monthly;
              method = rand(["bank_transfer", "card", "oxxo", "cash"]);
            } else {
              status = "overdue";
            }
          } else {
            ...
            if (rand100 < 0.55) {
              ...
              paidAmount = monthly;
              method = rand(["bank_transfer", "card", "oxxo"]);
            } else if (rand100 < 0.8) {
      ...
      // chunk insert (avoid > 1000 row payloads)
      for (let i = 0; i < rows.length; i += 500) {
        const chunk = rows.slice(i, i + 500);
        const { error } = await supabaseAdmin.from("payments").insert(chunk);
        if (error) throw new Error(`payments: ${error.message}`);
      }
      stats.payments = rows.length;
    }

    // 10. Incidents — 1 per ~5 policies, varied statuses
    {
      ...
      const rows = sample.map((p, idx) => {
        ...
        return {
          ...
          status,
          approved_at: status === "pass_issued" || status === "in_treatment" || status === "closed"
            ? shiftDays(occurred, 1).toISOString() : null,
          approved_by: status === "pass_issued" || status === "in_treatment" || status === "closed"
            ? context.userId : null,
          rejected_at: status === "rejected" ? shiftDays(occurred, 2).toISOString() : null,
          rejected_by: status === "rejected" ? context.userId : null,
